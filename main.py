...
# Enable CORS for all origins (safe for testing; restrict in production)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
def get_indicators(ticker: str = Query(...)):
    try:
        ...
        stochrsi_series = calculate_stochrsi(rsi_series)
        stochrsi_val = stochrsi_series.iloc[-1]

        # ✅ Return flat structure (fixes Thunkable parsing issues)
        return {
            "ticker": ticker,
